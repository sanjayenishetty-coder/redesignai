...
function FeedbackRow({ f, idx }: { f: FeedbackEntry; idx: number }) {
  ...
  const RatingBadge = ({ val }: { val: number | null }) => {
    ...
    return <span style={{ fontWeight: 700, color }}>{val}/5</span>;
  };
  return (
    <>
      <tr style={{ borderBottom: "1px solid #f3f4f6" }}>
        ...
        <td style={{ padding: "10px 12px", fontSize: 13, fontWeight: 600, color: "#111" }}>
          {f.participant_name || "Anonymous"}
        </td>
        {FEEDBACK_SPEAKERS.map((sp) => (
          <td key={sp.key} style={{ padding: "10px 12px", fontSize: 13, textAlign: "center" as const }}>
          ...
          <td colSpan={10} style={{ padding: "16px 20px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px 20px" }}>
              ...
              {FEEDBACK_SPEAKERS.filter(sp => sp.day === 1).map((sp) => {
                ...
                return (
                  <div key={sp.key}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase" as const, marginBottom: 4 }}>{sp.name}</div>
                    <div style={{ fontSize: 13, color: "#374151" }}>{response}</div>
                  </div>
                );
              })}
              {f.day2_feedback && (
        ...
        return (
          <div style={{ padding: "24px 32px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 17, color: "#111" }}>Workshop Feedback — REDESIGN-ai Day 1</div>
            ...
            {feedbackLoading ? (
              ...
              <>
                {/* Per-Speaker Avg Rating Cards */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 24 }}>
                  {FEEDBACK_SPEAKERS.map((sp) => {
                    const ratings = feedbackData.map((f) => (f as any)[`${sp.key}_rating`]).filter(Boolean) as number[];
                    return (
                      <div key={sp.key} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 10, padding: "16px", textAlign: "center" as const, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                        <div style={{ fontSize: 26, fontWeight: 800, color: "#007787" }}>{avg(ratings)}</div>
                        <div style={{ fontSize: 11, color: "#6b7280", marginTop: 4, lineHeight: 1.4 }}>{sp.name}</div>
                      </div>
                    );
                  })}
                </div>
